...
CHROMA_PATH = "chroma"
TEMPLATE = """
Ты — помощник, который обязан отвечать исключительно на основании предоставленного контекста.
Контекст:
{context}

Отвечай на следующий вопрос исходя из контекста, приведённого выше. Сохраняй стиль и тональность исходного текста в контексте (лексика, ритм, стиль изложения).
...
async def upload_file(file: UploadFile = File(...)):
    ...
    db = None
    db = Chroma.from_documents(chunks, OpenAIEmbeddings(model="text-embedding-3-small"), persist_directory=CHROMA_PATH,collection_name=COLLECTION_NAME)
...
async def answer(request: Request):
    ...
    db = Chroma(persist_directory=CHROMA_PATH,collection_name=COLLECTION_NAME,embedding_function= OpenAIEmbeddings(model="text-embedding-3-small"))
    fournn = db.similarity_search_with_relevance_scores(question_text, k=3)
